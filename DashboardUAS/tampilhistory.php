...
if(isset($_POST['update'])){
	...
	$sql = $database->updatehistory($Id_Pembelian, $Progress);

	if($sql){
		echo '<script>alert("Berhasil edit data."); document.location="tampilhistory.php";</script>';
	}else{
						...
						<form action="" method="post">
							<div class="modal-body">
								<div class="form-group">
									<label class="control-label" for="Id_Pembelian">Id Pembelian</label>
									<input type="text" name="Id_Pembelian" class="form-control" id="Id_Pembelian" required>
								</div>
								<div class="form-group">
				      	...
				      	<input type="submit" name="update" class="btn btn-primary" value="Update">
								<a href="tampilhistory.php" class="btn btn-warning">Kembali</a>
				      </div>
						</form>
					</div>
				</div>
						...
						if(mysqli_num_rows($sql) > 0){
							while($data = mysqli_fetch_assoc($sql))
							{
								...
								<tr>
									...
									<td>'.$data['Tanggal'].'</td>
									<td>		
										<a class="btn btn-success text-white" id="tombolubah" data-toggle="modal" data-target="#edit"  data-email="'.$data['Email_Plngn'].'" data-id="'.$data['Id_Pembelian'].'" data-progress="'.$data['Progress'].'" data-tanggal="'.$data['Tanggal'].'">Edit</a>
										<a href="hapushistory.php?Id_Pembelian='.$data['Id_Pembelian'].'" class="btn btn-secondary btn-danger" id="tombolhapus">Delete</a>
									</td>
								</tr>
								';
							}
